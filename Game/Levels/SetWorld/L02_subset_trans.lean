...
Statement subset_trans {S T U : Set 𝓧} (hST : S ⊆ T) (hTU : T ⊆ U) : S ⊆ U := by
  Hint "Start with `rw [subset_def] at *` to replace all occurences of the `⊆` symbol with
    their definitions."
  rw [subset_def] at *
  Hint "Now we've got to prove something for all elements of `𝓧`, so `intro a`
  to let `a` be an arbitrary element of `𝓧`."
  intro a
  Hint "Now we assume `a ∈ S` with `intro h`"
  intro h
...
Conclusion "Nice! Let's now prove antisymmetry, the third and final basic property of subsets."
